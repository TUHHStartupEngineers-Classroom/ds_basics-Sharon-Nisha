...
library(lubridate)
# Step 1 - Manipulate
sales_by_yearloc_nc <-sales_by_year_loc_tbl %>%
 
  # Select columns
  select(order.date,location,total.price) %>%
...
sales_by_yearloc_nc %>%
  ...
  ggplot(aes(x =year, y =total.price)) +
  
  # Geometries
  geom_col(fill = "#2DC6D6") + # Use geom_col for a bar plot
  geom_label(aes(label =location)) + # Adding labels to the bars
  geom_smooth(method = "lm", se = FALSE) + # Adding a trendline
  ...
  labs(
    title    = "Revenue by location and years",
    subtitle = "",
